...
export class AuthService {
  constructor(
    ...
    @InjectModel(ModelName.STUDENT)
    private readonly studentModel: Model<StudentDocument>,
    @InjectModel(ModelName.PERSONNEL)
  ...
  async checkUsername(checkUsernameDto: CheckUsernameDto) {
     ...
     * */
    const checkUser =
      (await this.userModel.findOne({ email: username })) ||
    ...
    throw new HttpException(
      'This user is not register into our system ',
      HttpStatus.NOT_FOUND,
    );
  }

  async signUp(signUpDto: SignUpDto) {
    const { email, phone_number, password, current_user_id, user_status } =
      signUpDto;
    ...
    if (!checkStudent && !checkPersonnel) {
      throw new HttpException(
        'This users information are not corresponds to any register student or personnel  into our system ',
        HttpStatus.UNAUTHORIZED,
      );
    }
